...
class Detector(nn.Module):
    ...
    def __init__(self):
        ...
        self.head = nn.Conv2d(in_channels=1280, out_channels=20, kernel_size=1)
        ...
        #   (rel_x_offset, rel_y_offset, rel_x_width, rel_y_height, confidence, [categories ...]
    ...
    def decode_output(self, out, threshold=None, topk=100):
        ...
        for o in out:
            ...
            #category_indices = torch.nonzeros(o[5:, bb_index[0], bb_index[1]] >= threshold)
            #print()
            ...
            for bb_index in bb_indices:
                ...
                categories = o[5:20, bb_index[0], bb_index[1]]
                #print(categories)
                categories = categories.numpy()

                cat = np.argmax(categories)
                #print(cat)
    ...
    def input_transform(self, image, anns):
        ...
        target = torch.zeros(int(5+self.categories), 15, 20)
        ...
        for ann in anns:
            ...
            #print(category)
            category_ind = int(category + 5)
            #print(category_ind)
            target[category_ind, y_ind, x_ind] = 1
